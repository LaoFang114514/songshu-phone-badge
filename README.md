#  松鼠兽牌

[![官方网站](https://img.shields.io/badge/官网-访问-blue.svg)](https://songshushoupai.mysxl.cn/)
[![GitHub](https://img.shields.io/badge/GitHub-仓库-181717?logo=github)](https://github.com/LaoFang114514/songshu-phone-badge)
[![GitCode](https://img.shields.io/badge/GitCode-仓库-C71D23?logo=git)](https://gitcode.com/LaoFang233/songshu-phone-badge)
[![爱发电赞助](https://img.shields.io/badge/赞助-爱发电-orange.svg)](https://www.ifdian.net/a/laofang)
[![Ko-fi赞助](https://img.shields.io/badge/赞助-Ko--fi-red.svg)](https://ko-fi.com/laofang)
[![开源协议](https://img.shields.io/badge/协议-GPL--3.0-green.svg)](https://www.gnu.org/licenses/gpl-3.0.html)
[![QQ交流群](https://img.shields.io/badge/QQ交流群-465174613-blue)](https://qun.qq.com/universal-share/share?ac=1&authKey=EHQoZ%2BHU4s8taqGbkhUKrWY4FAq2vLB2LoPRNMO7jLkxGyyNLrnWCNmaZ7DuJTOx&busi_data=eyJncm91cENvZGUiOiI0NjUxNzQ2MTMiLCJ0b2tlbiI6IjlHTnZHTlhwMWVlN2VEYXJzTitoWkZxSlV1VUhadnNsYzJNVkE5b0tIcGZvQW00TGxOK0lQRGFMaEZjeU5GZWEiLCJ1aW4iOiIyOTA4ODA3NzYwIn0%3D&data=LITJR0_gfVVBcvQQSGd-RQZ7xQ39dz8b0w_wJzZ69Z-mCigq-1uJsrRMOstu12BG7-aKJJu0EhSDGJprd-kbKQ&svctype=4&tempid=h5_group_info)

>前排提醒：仅适合备用机顶替使用，如果正经用的话建议购买别人的兽牌或电子兽牌（嗯）

一款简单易用的手机端兽牌制作工具，帮你把闲置旧手机变成专属兽牌展示牌，适合需要兽牌但经费不足的兽圈小伙伴。

---

## ✨ 功能特性
### 🖼️ 兽牌管理
- **图片 / 视频双模式** — 支持导入图片或视频作为兽牌，自由切换展示
- **新建与编辑** — 内置裁剪工具，可选择图片区域裁剪为兽牌
- **列表管理** — 重命名、上移、下移、删除，轻松管理多张兽牌
- **选中高亮** — 当前使用的兽牌带有主题色边框，一目了然

### 🎬 全屏播放
- **沉浸式体验** — 全屏展示兽牌，自动隐藏系统栏
- **手势交互** — 长按屏幕旋转兽牌（180° 翻转），双指捏合退出播放
- **二维码展示** — 上划屏幕弹出二维码，支持自定义二维码图片

### ⚙️ 实用设置
- **反向显示** — 启动时自动翻转 180°，适配倒置摆放
- **常亮显示** — 播放时屏幕保持常亮，不自动熄灭
- **电量显示** — 全屏时顶部显示当前电量百分比
- **锁定方向** — 禁止长按旋转，固定兽牌朝向
- **防烧屏** — 每隔 5 分钟轻微移动画面，防止 OLED 残影

### 💾 数据备份
- **本地导入/导出** — 配置和图片打包为 ZIP 文件，方便迁移
- **WebDAV 云备份** — 支持上传到 WebDAV 服务器或从服务器恢复


---

## 📱 系统要求

- **最低版本**：Android 5.0 
- **推荐版本**：Android 8.0 及以上
- **架构支持**：armeabi-v7a / arm64-v8a

> 只在 32 位 Android 6.0 设备上测试通过，其他手机不确定能不能运行。

---


## 💖 支持作者

如果本工具对你有帮助，欢迎通过以下渠道赞助：

- **国内赞助（爱发电）**：[ifdian.net/a/laofang](https://www.ifdian.net/a/laofang)
- **海外赞助（Ko-fi）**：[ko-fi.com/laofang](https://ko-fi.com/laofang)

---

## 📄 开源协议

本项目基于 **GNU General Public License v3.0** 协议开源，详见 [LICENSE](LICENSE) 文件。

Copyright (C) 2026 laofang
